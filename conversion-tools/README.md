...
`Test-UnityLogicReconnect.ps1` performs bounded, repeatable Windows Player fault tests without UI automation. It launches one Player with a dedicated Unity log and accepts three mutually exclusive fault modes:
...
Both modes find the single `ThreeUnityWebHost.exe` whose `ParentProcessId` and `--parent-pid` both match that newly launched Player. Every polling interval rechecks this exact relationship and fails if more than one matching Host is alive at once.
...
2. `THREE_UNITY_WEB_BRIDGE_RELAUNCH_SCHEDULED`;
3. `THREE_UNITY_WEB_BRIDGE_RELAUNCHED`;
4. `THREE_UNITY_LOGIC_TRANSPORT_RESET`;
5. a second `THREE_UNITY_LOGIC_READY`;
6. a subsequent `THREE_UNITY_LOGIC_TICK`.
...
`-KillHost` and `-HangBeforeConnect` each belong to a separate PowerShell parameter set from the default `-SuspendMilliseconds` / `-SkipInputStale` mode. The shell therefore rejects mixed fault modes before launching anything. `-HangMilliseconds` is valid only with `-HangBeforeConnect`.
